...
class Player:
    ...
    def main(self):
        ...
        def dice_value():
            msg = self.server.recv(1024).decode("utf-8").split(":")
            # print(msg)
        ...
        class column_stack:
            ...
            def receiving_light(self, which_piece):
                ...
                if (
                    (len(self.elements) == 0)
                    or (len(self.elements) == 1 and self.elements[0].id == opponent)
                    or (len(self.elements) < 6 and self.elements[0].id == which_piece)
                ):
                    if self.location < 13:
        ...
        stack6 = column_stack(
            6, white_piece1, white_piece2, white_piece3, white_piece4, white_piece5
        )
        ...
        stack12 = column_stack(
            12, black_piece9, black_piece10, black_piece11, black_piece12, black_piece13
        )
        stack13 = column_stack(
            13, white_piece13, white_piece12, white_piece11, white_piece10, white_piece9
        )
        ...
        stack19 = column_stack(
            19, black_piece5, black_piece4, black_piece3, black_piece2, black_piece1
        )
        ...
        while running:
            ...
            # making lights on if self.turn is not completed
            ...
            if self.color == "white":
                # when turn of player(you) is completed
                if ((player_dice1_moved == True) and (player_dice2_moved == True)) or (
                    (len(white_legal_destination) == 0)
                    and (
                        you_dice_rolled == True
                        and (
                            (player_dice1_moved == player_dice2_moved == False)
                            or (player_dice1_moved == True)
                            or (player_dice2_moved == True)
                        )
                    )
                ):
                    player_dice1_moved = False
                ...
                if (
                    ((cpu_dice1_moved == True) and (cpu_dice2_moved == True))
                    or (
                        (len(black_legal_destination) == 0)
                        and (
                            cpu_dice_rolled == True
                            and (
                                (cpu_dice1_moved == cpu_dice2_moved == False)
                                or (cpu_dice1_moved == True)
                                or (cpu_dice2_moved == True)
                            )
                        )
                    )
                    or (self.cpu_number_of_peice_to_move == 0)
                ):
                    cpu_dice_rolled = False
                    cpu_dice1_moved = False
                    cpu_dice2_moved = False
            ...
            if self.color == "black":
                # when turn of player(you) is completed
                if (
                    ((player_dice1_moved == True) and (player_dice2_moved == True))
                    or (
                        (len(white_legal_destination) == 0)
                        and (
                            you_dice_rolled == True
                    ...
                    or (self.you_number_of_peice_to_move == 0)
                ):
                    player_dice1_moved = False
                    player_dice2_moved = False
                    light_trigerred = False
                ...
                # when turn of cpu is completed
                if ((cpu_dice1_moved == True) and (cpu_dice2_moved == True)) or (
                    (len(black_legal_destination) == 0)
                    and (
                        cpu_dice_rolled == True
                        and (
            ...
            for event in pg.event.get():
                ...
                if self.turn == "you":  # find
                    for i in white_light_pieces:
                        ...
                        if (
                            ...
                            and (event.key == pg.K_m or event.key == pg.K_SPACE)
                            and you_dice_rolled
                        ):
                            if (
                                ...
                                if event.key == pg.K_m:
                                    if len(white_reached_home) <= 15:
                                        if d1 > 0 and player_dice1_moved == False:
                                            ...
                                            if (
                                                all_stack_dict[d1]
                                                in white_legal_destination
                                            ):
                                                if len(
                                                    all_stack_dict[d1].elements
                                                ) == 1 and (
                                                    all_stack_dict[d1].elements[0].id
                                                    == "black"
                                                ):
                                                    move(
                                                        all_stack_dict[d1],
                                                ...
                                                if all_stack_dict[d1].location <= 6:
                                                    if (
                                                        all_stack_dict[d1].elements[-1]
                                                        not in white_reached_home
                                                    ):
                                                        white_reached_home.append(
                                                            all_stack_dict[d1].elements[
                                                                -1
                                                            ]
                                                        )
                                        ...
                                        if (
                                            d1 == 0
                                            and player_dice1_moved == False
                                            and len(white_reached_home) == 15
                                        ):
                                            # print("M is pressed")
                                    ...
                                    if len(white_reached_home) <= 15:
                                        if d2 > 0 and player_dice2_moved == False:
                                            ...
                                            if (
                                                ...
                                                if len(
                                                    all_stack_dict[d2].elements
                                                ) == 1 and (
                                                    all_stack_dict[d2].elements[0].id
                                                    == "black"
                                                ):
                                                    move(
                                                        all_stack_dict[d2],
                ...
                if self.turn == "cpu":
                    for i in black_light_pieces:
                        ...
                        # move_from, move_to = receive_message(peer2)
                        # move(move_from, move_to)  # find
                        if (
                            event.type == pg.KEYDOWN
                            and (event.key == pg.K_m or event.key == pg.K_SPACE)
                            and cpu_dice_rolled
                            ...
                            if (
                                ...
                                if event.key == pg.K_m:
                                    if len(black_reached_home) <= 15:
                                        if d1 < 25 and cpu_dice1_moved == False:
                                            ...
                                            if (
                                                ...
                                                if all_stack_dict[d1].location >= 19:
                                                    if (
                                                        all_stack_dict[d1].elements[-1]
                                                        not in black_reached_home
                                                    ):
                                                        black_reached_home.append(
                                                            all_stack_dict[d1].elements[
                                                                -1
                                                            ]
                                                        )
                                                ...
                                                dice1_cpu.my_dice = pg.image.load(
                                                    blank_cpu
                                                )
                                        ...
                                        if (
                                            d1 == 25
                                            and cpu_dice1_moved == False
                                            and len(black_reached_home) == 15
                                        ):
                                            # print("M is pressed")
                                        ...
                                        if d2 < 25 and cpu_dice2_moved == False:
                                            ...
                                            if (
                                                ...
                                                if all_stack_dict[d2].location >= 19:
                                                    ...
                                                    # print(len(black_reached_home))
                                                    # print(black_reached_home)
                                                dice2_cpu.my_dice = pg.image.load(
                                                    blank_cpu
                                                )
            ...
            if (
                start_clicked == True
                and how_play_understood == True
                and rules_understood == False
            ):
            ...
            if self.you_turn_msg:
                screen.blit(having_first_move, (19, 68))
                # pg.time.delay(1000)
                # self.you_turn_msg = False

            if self.cpu_turn_msg:
